...
for i = 1:(tsteps-tstart+1) 
    indices(i)=indices(i)-minimum;
    disp(i)
    
    for j = 1:(nparticipants*trials) 
    syncshape = tsteps - indices(i)+1;
    LFvector = LFtable(indices(i):end,j);
    alignedLFtable(1:length(LFvector),j)= LFvector;
    GFvector = GFtable(indices(i):end,j);
    alignedGFtable(1:length(GFvector),j)= GFvector;    
    end 
