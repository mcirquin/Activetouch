function [LF_pvalues,LF_indexes,GF_pvalues,GF_indexes] = plotmeancurvesdoublecatch(alignedLFtable, alignedGFtable, tsteps, nparticipants,minimum)
%% Double catch low friction (high to low friction)
...
%% Double catch high friction (low to high friction)
...
legend('', '')
title('Maximal manipulandum weight catch')
xlabel('Time (s)');
ylabel('LF (N)');
xlim([-timealigned 2]);
ylim([0 LFaxislim]);
...
title('Minimal manipulandum weight catch')
xlabel('Time (s)');
ylabel('LF (N)');
ylim([0 LFaxislim]);
xlim([-timealigned 2]);
...
suptitle( 'Adaptation to friction during the first movement of double catch trials - Elderly participants');
